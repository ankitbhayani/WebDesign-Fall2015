...
<%
		...
		try
		{
			...
			//out.println(" Inside login " +userName);
			//out.println("\n Inside userPassword " +userPassword);
			String pswd = "";
            ...
            rs = stmt.executeQuery("SELECT UserPassword FROM userstable WHERE UserName='" + userName + "'");
			 ...
			 if (!rs.isBeforeFirst() ) {
             			throw new RuntimeException("Sorry No records with this user name");
             }	
             else{
				   while (rs.next())
                	{
            ...
            if (!userPassword.equals(pswd))
            	throw new RuntimeException("Wrong Combination! Please check your password");
			 }
